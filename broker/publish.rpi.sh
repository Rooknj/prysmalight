...
buildDocker() {
    ...
    echo "Tagging docker image with \"broker-rpi-$tag\""
    docker tag lightapp2-broker-rpi rooknj/lightapp2:broker-rpi-$tag

    echo "Pushing docker image"
    docker push rooknj/lightapp2:broker-rpi-$tag
}
...
getConfirmationFromUser() {
    printf "I will tag this docker image with \"broker-rpi-$tag\", is that ok? [Y/n]: " 
    read confirm
}
